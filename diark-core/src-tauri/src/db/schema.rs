...
/// Tạo schema lưu trữ lịch thi và danh mục đồ dùng phòng thi (idempotent).
pub fn ensure_exam_schema(conn: &Connection) -> SqlResult<()> {
    conn.execute_batch(
        r#"
        ...
        CREATE INDEX IF NOT EXISTS idx_academic_exams_timestamp ON academic_exams(exam_timestamp);
        CREATE INDEX IF NOT EXISTS idx_academic_exams_subject ON academic_exams(subject_code);
        "#,
    )?;
    Ok(())
}
