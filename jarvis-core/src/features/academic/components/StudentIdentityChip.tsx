...
export function StudentIdentityChip() {
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs text-slate-400 shadow-sm">
      <span className="font-mono font-medium text-slate-200">{profile.full_name}</span>
      <span className="text-slate-600">•</span>
      <span className="font-mono text-slate-300">{profile.student_id}</span>
      <span className="text-slate-600">•</span>
      <span className="text-cyan-400/90">{profile.student_class}</span>
      <span className="text-slate-600">•</span>
